...
//! Codegen: turn `crates/cyrs-ast/cypher.ungrammar` into
//! `crates/cyrs-ast/src/generated.rs` (spec 0001 §5.1, §5.2).
//!
//! The generator emits zero-cost wrapper structs around
//! `cyrs_syntax::SyntaxNode` — the rust-analyzer pattern. One wrapper
//! per named production whose `SyntaxKind` variant exists in
//! `cyrs-syntax::kind`. Productions whose variant is missing get listed
...
/// `crates/cyrs-ast/src/generated.rs`.
pub fn run() -> Result<()> {
...
pub fn build_generated_rs_from(ungrammar_src: &str) -> Result<CodegenReport> {
    ...
    // `cyrs-syntax::kind` to be emittable; enum-shaped productions have
    ...
    for node_ref in grammar.iter() {
        ...
        if !kind_variants.contains(kind.as_str()) {
            skipped.push(SkippedProduction {
                name: name.clone(),
                reason: format!(
                    "no `SyntaxKind::{kind}` variant in cyrs-syntax::kind (see cy-nom follow-ups)"
                ),
            });
            continue;
        }
...
fn render_file(
    ...
    out.push_str("// Source: crates/cyrs-ast/cypher.ungrammar (spec 0001 §5.2).\n");
    out.push_str("//\n");
    out.push_str("// Typed AST wrappers are zero-cost views over `cyrs_syntax::SyntaxNode`\n");
    ...
    out.push('\n');
    out.push_str("use cyrs_syntax::{SyntaxElement, SyntaxKind, SyntaxNode, SyntaxToken};\n");
    out.push('\n');
...
/// (`SOURCE_FILE..ERROR` per `cyrs-syntax::kind`). Kept in sync by the
...
/// Keyword zone of `cyrs-syntax::kind`. A keyword accessor is only
...
fn ast_ungrammar_path() -> Result<PathBuf> {
    Ok(workspace_root()?.join("crates/cyrs-ast/cypher.ungrammar"))
}

fn ast_generated_path() -> Result<PathBuf> {
    Ok(workspace_root()?.join("crates/cyrs-ast/src/generated.rs"))
}
