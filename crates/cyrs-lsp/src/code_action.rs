...
//!
//! Surfaces `cyrs_diag::FixIt` entries on diagnostics that intersect
...
use cyrs_diag::Diagnostic;
use cyrs_syntax::{LineIndex, TextRange, TextRangeExt, TextSize};
use lsp_types::{
...
mod tests {
    ...
    //! `Diagnostic::fixes` (the shape exists in `cyrs-diag` but no
    ...
    //! covered.
    use super::*;
    use cyrs_diag::{Applicability, DiagCode, Diagnostic, FixIt, TextEdit as DiagEdit};
