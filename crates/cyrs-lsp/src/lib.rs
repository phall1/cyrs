...
//!
//! The crate ships a binary (`cyrs-lsp`) that wires `Connection::stdio()`
...
pub(crate) struct Server {
    ...
    pub(crate) project: Option<cyrs_project::ProjectManifest>,
...
pub(crate) fn ensure_project_loaded(server: &mut Server, hint: &std::path::Path) {
    ...
    let Some(manifest_path) = cyrs_project::discover(&dir) else {
        return;
    };
    let manifest = match cyrs_project::load_from_toml_path(&manifest_path) {
        Ok(m) => m,
        Err(e) => {
        ...
        && let Some(ref schema) = manifest.schema
    {
        ...
        server.db.set_schema(Some(
            Arc::new(schema.clone()) as Arc<dyn cyrs_schema::SchemaProvider>
        ));
    }
...
fn handle_execute_command(
    ...
    match params.command.as_str() {
        "cypher.explainPlan" => {
            ...
            match server.db.plan_of(file_id) {
                Ok(plan) => Response::new_ok(
                    id,
                    serde_json::Value::String(cyrs_plan::pretty::pretty(plan.plan())),
                ),
                Err(e) => Response::new_err(
                    id,
                    lsp_server::ErrorCode::InternalError as i32,
        ...
        "cypher.lowerToHir" => {
            ...
            Response::new_ok(
                id,
                serde_json::Value::String(cyrs_hir::pretty::print_overlay(&stmt, &resolved)),
            )
        }
...
fn handle_range_formatting(
    ...
    let formatted = cyrs_fmt::format(&source);
    if formatted == source {
...
fn references_workspace(
    db: &Database,
    project: &cyrs_project::ProjectManifest,
    ...
    let locs = cyrs_lang_services::find_references(db, project, file_id, offset);
    if locs.is_empty() {
        return None;
    }

    // Optional declaration drop.
    let decl = cyrs_lang_services::goto_definition(db, project, file_id, offset);
...
fn definition_workspace(
    ...
    let line_index = LineIndex::new(&source);
    let offset = position_to_offset(&line_index, position)?;
    let loc = cyrs_lang_services::goto_definition(db, project, file_id, offset)?;
...
fn handle_workspace_symbol(
    ...
    let results = cyrs_lang_services::workspace_symbols(&server.db, project, params.query.as_str());
...
fn symbol_kind_to_lsp(kind: cyrs_lang_services::SymbolKind) -> lsp_types::SymbolKind {
    match kind {
        cyrs_lang_services::SymbolKind::Label => lsp_types::SymbolKind::CLASS,
        cyrs_lang_services::SymbolKind::RelType => lsp_types::SymbolKind::INTERFACE,
        cyrs_lang_services::SymbolKind::Param => lsp_types::SymbolKind::VARIABLE,
        cyrs_lang_services::SymbolKind::NamedPath => lsp_types::SymbolKind::KEY,
...
fn text_range_to_lsp(line_index: &LineIndex, range: cyrs_syntax::TextRange) -> lsp_types::Range {
    lsp_types::Range {
...
mod tests {
    ...
    fn open(server: &mut Server, uri: &str, source: &str) -> cyrs_db::workspace::FileId {
        let file_id = server
            .db
