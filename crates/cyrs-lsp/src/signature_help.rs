...
use cyrs_schema::{FunctionSignature, ParamDecl, PropertyType};
...
mod tests {
    ...
    /// exercised without depending on cyrs-testkit (dev-dep cycle) or
    /// `cyrs-schema::StandardLibrary` (not part of the v1 surface yet).
        ...
        fn function(&self, name: &str) -> Option<FunctionSignature> {
            if name == "toInteger" {
                Some(FunctionSignature {
                    ...
                    params: vec![cyrs_schema::ParamDecl {
                        name: SmolStr::new("value"),
                        ty: cyrs_schema::PropertyType::String,
                        default: None,
                    }],
                    variadic: None,
                    return_ty: ReturnTy::Constant(cyrs_schema::PropertyType::Int),
    ...
    fn db_with_source(source: &str) -> (Database, cyrs_db::FileId) {
