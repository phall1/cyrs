//! `cyrs-agent` — JSON-over-stdio agent API. Spec 0001 §15.
//!
...
use cyrs_diag::json as diag_json;
...
use cyrs_plan::pretty::pretty as plan_pretty;
use cyrs_schema::SchemaProvider;
use cyrs_syntax::TextSize;
...
// ---------------------------------------------------------------------------
// In-memory schema loaded from schema_set JSON (mirrors cyrs-testkit)
// ---------------------------------------------------------------------------

/// A minimal `SchemaProvider` deserialized from the agent's `schema_set` JSON.
/// Mirrors the `JsonSchema` in `cyrs-testkit` so the wire shape is identical.
...
fn parse_prop_type(s: &str) -> cyrs_schema::PropertyType {
    match s {
        "String" => cyrs_schema::PropertyType::String,
        "Int" => cyrs_schema::PropertyType::Int,
        "Float" => cyrs_schema::PropertyType::Float,
        "Bool" => cyrs_schema::PropertyType::Bool,
        "Date" => cyrs_schema::PropertyType::Date,
        "Datetime" => cyrs_schema::PropertyType::Datetime,
        _ => cyrs_schema::PropertyType::Any,
    }
}
...
impl SchemaProvider for AgentSchema {
    ...
    fn node_properties(&self, label: &str) -> Option<Vec<cyrs_schema::PropertyDecl>> {
    ...
    fn relationship_properties(&self, rel_type: &str) -> Option<Vec<cyrs_schema::PropertyDecl>> {
    ...
    fn relationship_endpoints(&self, rel_type: &str) -> Vec<cyrs_schema::EndpointDecl> {
        self.rel_endpoints
            .get(rel_type)
            .map_or_else(Vec::new, |eps| {
                eps.iter()
                    .map(|ep| cyrs_schema::EndpointDecl {
    ...
    fn function(&self, name: &str) -> Option<cyrs_schema::FunctionSignature> {
        self.functions.iter().find(|f| f.name == name).map(|f| {
            let params = f
                .params
                .iter()
                .map(|p| cyrs_schema::ParamDecl {
                    name: smol_str::SmolStr::new(p),
                    ty: cyrs_schema::PropertyType::Any,
                    default: None,
                })
                .collect();
            let variadic = f.variadic.as_deref().map(|_| cyrs_schema::ParamDecl {
                name: smol_str::SmolStr::new("..."),
                ty: cyrs_schema::PropertyType::Any,
                default: None,
            });
            cyrs_schema::FunctionSignature {
                name: smol_str::SmolStr::new(&f.name),
                params,
                variadic,
                return_ty: cyrs_schema::ReturnTy::Constant(parse_prop_type(&f.return_type)),
                categories: cyrs_schema::FnCategories {
                    pure: true,
                    aggregate: false,
                    deterministic: true,
                },
    ...
    fn procedure(&self, name: &str) -> Option<cyrs_schema::ProcedureSignature> {
        self.procedures.iter().find(|p| p.name == name).map(|p| {
            let params = p
                .params
                .iter()
                .map(|param| cyrs_schema::ParamDecl {
            ...
            let yields = p
                .yield_columns
                .iter()
                .map(|col| cyrs_schema::YieldDecl {
                    name: smol_str::SmolStr::new(col),
                    ty: cyrs_schema::PropertyType::Any,
                })
                .collect();
            cyrs_schema::ProcedureSignature {
...
fn handle(
    ...
    match req {
        ...
        // ------------------------------------------------------------------
        // complete — delegated to cyrs-lang-services (cy-da0 / cy-4t0).
        // ------------------------------------------------------------------
        ...
        // ------------------------------------------------------------------
        // hover — delegated to cyrs-lang-services (cy-o59 / cy-4t0).
        // ------------------------------------------------------------------
        AgentRequest::Hover {
            text,
            offset,
        ...
        // ------------------------------------------------------------------
        // rewrite — delegated to cyrs-lang-services (cy-taz / cy-4t0).
...
// ---------------------------------------------------------------------------
// Neutral → JSON adapters for cyrs-lang-services DTOs
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    fn intern_file_1000_requests_single_fileid() {
        ...
        let gap = cyrs_db::FileId(first.0.wrapping_add(1));
        assert!(
            !db.is_open(gap),
