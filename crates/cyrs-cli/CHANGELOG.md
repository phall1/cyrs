...
All notable changes to `cyrs-cli` are documented here.  The format is based
...
- cy-atk (§16): `cypher check` now runs the full analysis pipeline via
  ...
  with source underline (via `cyrs-diag::render_text_stderr`). Exit
...
- cy-atk (§16): switched `cyrs-cli` from `LegacyDatabase` to the
