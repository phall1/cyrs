...
//!
//! Converts `cyrs-lang-services`' workspace symbol index
//! ([`cyrs_lang_services::WorkspaceSymbolIndex`] — see spec §14.2,
...
//! ```
//!
//! * `<path>` is walked via [`cyrs_project::discover`] to find the
...
//! cypher-frontend cyrs-project  <project>    <version|HEAD>   <kind>/<name>#
//! ```
//!
...
use cyrs_lang_services::{Location, SymbolKind as CyKind, WorkspaceSymbolIndex, build_index};
use cyrs_project::{DialectDefault, ProjectManifest};
...
/// mirror the ecosystem's real-world "cyrs-project" manifest name.
const PACKAGE_MANAGER: &str = "cyrs-project";

/// Tool name surfaced in the SCIP `Metadata.tool_info` field.
const TOOL_NAME: &str = "cyrs-cli";
...
pub fn emit_index(path: &Path) -> Result<Index> {
    let manifest_path = cyrs_project::discover(path).with_context(|| {
        format!(
    ...
    let mut loaded: Vec<(cyrs_db::FileId, PathBuf, String)> =
...
fn build_scip(
    ...
    loaded: &[(cyrs_db::FileId, PathBuf, String)],
) -> Index {
...
fn scip_kind_for(kind: CyKind) -> symbol_information::Kind {
    match kind {
        ...
        // `cyrs_lang_services::SymbolKind` means we must handle
...
///
/// We avoid dragging `cyrs_syntax::LineIndex` into this crate
/// (cyrs-cli's edge list only admits `cyrs-db`, `-diag`, `-fmt`,
...
fn encode_symbol(project: &str, version: &str, kind: CyKind, name: &str) -> String {
    ...
    // `cypher-frontend cyrs-project <project> <version> <kind>/<name><suffix>`
