...
use cyrs_diag::{Severity, render_text_stderr};
...
fn schema_load(path: &Path) -> u8 {
    match cyrs_schema::file::load_from_toml_path(path) {
        Ok(schema) => {
            println!(
...
/// issue was reported.
///
/// [`SchemaLint`]: cyrs_schema::lint::SchemaLint
fn schema_check(path: &Path) -> u8 {
    let schema = match cyrs_schema::file::load_from_toml_path(path) {
        Ok(s) => s,
        Err(err) => {
            eprintln!("error: {err}");
    ...
    let lints = cyrs_schema::lint::lint(&schema);
    let mut had_error = false;
    for lint in &lints {
        if lint.severity == cyrs_schema::lint::LintSeverity::Error {
            had_error = true;
        }
        eprintln!("{lint}");
    }
...
///
/// [`SchemaDiff`]: cyrs_schema::diff::SchemaDiff
fn schema_diff(old: &Path, new: &Path) -> Result<u8> {
    let old_schema = cyrs_schema::file::load_from_toml_path(old)
        .with_context(|| format!("loading old schema {}", old.display()))?;
    let new_schema = cyrs_schema::file::load_from_toml_path(new)
        .with_context(|| format!("loading new schema {}", new.display()))?;
    let report = cyrs_schema::diff::diff(&old_schema, &new_schema);
...
fn project_load(path: &Path) -> u8 {
    match cyrs_project::load_from_toml_path(path) {
        Ok(m) => {
            let dialect = match m.dialect.default {
                cyrs_project::DialectDefault::GqlAligned => "GqlAligned",
                cyrs_project::DialectDefault::OpenCypherV9 => "OpenCypherV9",
            };
