//! `cypher-py` — Python (`PyO3`) binding for the Cyrs agent op surface.
//! Spec 0004 §6.
//!
//! This crate is a **thin adapter** (spec 0004 §2.1) over
//! [`cypher_lang_services`], [`cypher_db`], [`cypher_diag`], and
//! [`cypher_fmt`].  No parsing, sema, formatting, or planning logic
...
//! # Binding choice
//!
//! `PyO3` binds directly to [`cypher_lang_services`] — it does **not**
//! route through `cypher-ffi` (spec 0004 §6.1).  `PyO3` handles error
...
//! [`cypher_db::Database`] plus a single interned `FileId` reused
...
use cypher_schema::file::load_from_toml_str;
...
fn diag_from(idx: &LineIndex, d: &cypher_diag::Diagnostic) -> Diagnostic {
...
impl CypherDatabase {
    ...
    // ----------------------------------------------------------------------
    // complete / hover — delegated to cypher-lang-services.
    // ----------------------------------------------------------------------
    ...
    ///
    /// Delegates to [`cypher_schema::file::load_from_toml_str`] so the
