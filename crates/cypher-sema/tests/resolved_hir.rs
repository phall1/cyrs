...
//!   1. Parse → AST (via [`cypher_hir::lower::lower_statement`])
//!   2. Lower → HIR
//!   3. Desugar ([`cypher_hir::desugar::desugar_statement`])
//!   4. Resolve names ([`cypher_sema::resolve`])
//!   5. Snapshot the HIR with the resolved-name overlay
//!      ([`cypher_hir::pretty::print_overlay`])
//!
...
fn fmt_overlay_with_diags(overlay: String, diags: &[cypher_diag::Diagnostic]) -> String {
...
fn pipeline_stmt_with_kinds(stmt: Statement, warn_shadowing: bool) -> String {
    let mut sink = DiagnosticsSink::new();
    let result = resolve(&stmt, warn_shadowing, &mut sink);
    cypher_sema::kinds::check_kinds(&stmt, &mut sink);
    let diags = sink.into_sorted();
    let overlay = print_overlay(&stmt, &result.resolved_names);
    fmt_overlay_with_diags(overlay, &diags)
}
