...
//! between an embedder-owned catalog and `cypher-sema`'s schema-aware
//! analysis pass.
//!
//! Run it with:
//!
...
//! 2. The same query against [`cypher_schema::EmptySchema`] — every
...
//!
//! The split makes it obvious which checks `cypher-sema` runs (schema-
...
use cypher_hir::{desugar::desugar_statement, lower::lower_statement, scope::Resolution};
use cypher_schema::{
    Cardinality, EmptySchema, EndpointDecl, FunctionSignature, ProcedureSignature, PropertyDecl,
    PropertyType, SchemaProvider,
};
use cypher_sema::{SemaOptions, analyse, resolve};
...
fn run_with(label: &str, schema: Option<&dyn SchemaProvider>) {
    ...
    // bound. `cypher-sema::analyse` runs the same resolver pass
