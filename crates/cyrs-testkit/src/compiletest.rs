...
use cyrs_sema::{DialectMode, SemaOptions, analyse};
...
///
/// Invokes `cyrs_syntax::parse`, formats syntax errors as diagnostics,
...
///
/// Invokes `cyrs_hir::lower_statement + desugar_statement` then
/// `cyrs_sema::analyse`, renders all diagnostics, and compares against
...
///
/// Invokes the full HIR pipeline then `cyrs_plan::lower_statement`, renders
...
///
/// Invokes `cyrs_fmt::format` and compares the result against
/// `.formatted.cypher`.
///
/// # Errors
/// Returns an error if the input file or sidecar cannot be read/written.
