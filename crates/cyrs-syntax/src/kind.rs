...
pub enum SyntaxKind {
    ...
    // LAST_KW); the leading NULLS_KW is the introducer.  Slot 420 — 415
    // through 419 are reserved by sister beads (cy-p3cl claims 419 for
    // `LABEL_PAREN_EXPR`).
    NULL_ORDERING = 420,
    // --- end cy-z0x8 ---
...
impl SyntaxKind {
    ...
    /// Returns `true` for the keyword zone (`MATCH_KW..=LAST_KW`).
    ///
    /// `LAST_KW` is the current upper bound (cy-z0x8 OFFSET + NULLS
    /// FIRST/LAST, slots 198..=201, above cy-71t0's `GROUP_KW` at 197).
    /// Internal slots may be unassigned — `from_u16` returns `None`
    /// for those gaps, so they never round-trip into a keyword.
    #[must_use]
    pub const fn is_keyword(self) -> bool {
        let k = self as u16;
        k >= Self::MATCH_KW as u16 && k <= Self::LAST_KW as u16
    }
