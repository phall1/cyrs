...
//!
//! All server logic lives in the `cypher_lsp` library module (`src/lib.rs`).
//! This binary just wires tracing + `Connection::stdio()` to
//! [`cypher_lsp::serve`] so integration tests can reuse the same server
...
fn main() -> Result<()> {
    ...
    let result = cypher_lsp::serve(&connection);
    io_threads.join()?;
