...
//! `cypher_hir::Statement` contains a `node_map: IndexMap<HirId, SyntaxNode>`
...
use cypher_sema::resolve::ResolveResult;
...
/// schema does not.
///
/// In v1 the typed AST wrappers from `cypher-ast` are zero-cost views over
...
pub fn resolved_names(
    ...
    let result = cypher_sema::resolve::resolve(&stmt, opts.warn_shadowing, &mut sink);
...
pub fn sema_diagnostics(
    ...
    let sema_opts = SemaOptions {
        ...
        dialect: match opts.dialect {
            DialectMode::GqlAligned => cypher_sema::DialectMode::GqlAligned,
            DialectMode::OpenCypherV9 => cypher_sema::DialectMode::OpenCypherV9,
        },
    };

    let mut sink = DiagnosticsSink::new();
    cypher_sema::analyse(&stmt, schema_ref, &sema_opts, &mut sink);
...
mod tests {
    ...
    fn schema_change_reruns_sema_only() {
        ...
        let schema: Arc<dyn cypher_schema::SchemaProvider> = Arc::new(EmptySchema);
