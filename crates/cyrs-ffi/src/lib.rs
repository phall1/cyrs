//! `cyrs-ffi` — stable C ABI over the Cyrs Cypher / GQL frontend.
//! Spec 0004 §5.
//!
...
#![doc(html_root_url = "https://docs.rs/cyrs-ffi/0.0.1")]
...
use cyrs_diag::{Diagnostic, Severity};
...
/// [`cyrs_db::Database`] plus a single `FileId` interned on first use;
/// mirrors `cyrs-wasm`'s per-dialect file-registry but simplified to
...
/// `cyrs-plan`, which is not part of the FFI allow-list (spec 0004 §3).
/// When the neutral `plan_json` helper lands in `cyrs-lang-services`
