//! `cyrs-sema` — semantic analysis and type system (spec 0001 §7).
//!
//! Two modes (§7.1):
//!
...
//!   type errors.
//! - **Schema-aware.** Run when a [`cyrs_schema::SchemaProvider`] is
...
//! can therefore upgrade `cyrs-sema` minor versions without
//! touching their adapter code. The migration template at
//! `crates/cyrs-sema/examples/embedder_adapter.rs` is part of the
...
#![forbid(unsafe_code)]
#![doc(html_root_url = "https://docs.rs/cyrs-sema/0.0.1")]
...
pub use lints::{LintOptions, run_lints};
...
mod tests {
    use super::*;
    use cyrs_hir::Statement as HirStmt;
