...
//! `actual` for later conversion to a [`cyrs_diag::Diagnostic`] with
//! code [`cyrs_diag::DiagCode::E5003`].
...
impl TypeMismatch {
    /// Convert this mismatch to a [`cyrs_diag::Diagnostic`].
    ///
    /// The caller supplies the source range because `unify` is a pure function
    /// without span information. Code is always `E5003`.
    pub fn into_diagnostic(self, range: cyrs_syntax::TextRange) -> cyrs_diag::Diagnostic {
        use cyrs_diag::{DiagCode, Diagnostic, Label};
        use smol_str::SmolStr;
