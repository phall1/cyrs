...
`crates/cyrs-py/python/cypher/__init__.pyi`. Every PyO3-exported
...
cyrs-py` exercises the plain-Rust helper paths (severity mapping,
completion-kind mapping, FileId interning) directly. `cargo build -p
cyrs-py` fails at link on hosts without a Python toolchain (PyO3's
