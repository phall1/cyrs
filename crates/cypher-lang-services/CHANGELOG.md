# Changelog — `cypher-lang-services`

All notable changes to `cypher-lang-services` are documented here.  The format
...
- `cy-4t0` (§14, §15): initial crate — lifts shared completion, hover, and
  rewrite engines out of `cypher-lsp` and `cypher-agent` so both binaries
