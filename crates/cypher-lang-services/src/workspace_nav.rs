...
//! `workspace/symbol` requests.  The LSP adapter layer in `cypher-lsp`
...
//! `cypher-db`) can cache the index itself once the tracked-query
...
use cypher_project::ProjectManifest;
...
mod tests {
    use super::*;
    use cypher_db::{Database as Db, DialectMode};
    use cypher_project::{DialectConfig, DialectDefault, ProjectManifest};
