...
//! the 5-byte input `MATCH` round-trips through `cyrs_hir::lower` as a
//! `Clause::Match` whose [`cyrs_hir::PatternPart`] carries zero
//! [`cyrs_hir::PatternElement`]s. The previous lowerer answered that
//! shape with `.expect("pattern part must have at least one element")`
//! at `crates/cyrs-plan/src/lower.rs:682`. The fix surfaces the
//! recovery shape as `PlanLowerError::EmptyPatternPart { .. }` (see
//! `cyrs_plan::error`) instead.

#![cfg(test)]

use cyrs_hir::{Clause, HirId, HirSpan, Pattern, PatternElement, PatternPart, Statement};
use cyrs_plan::PlanLowerError;
use cyrs_plan::lower::lower_statement;

/// Lowering a `MATCH` clause with a single empty pattern part must
/// surface `EmptyPatternPart` rather than panicking.
///
/// This shape is exactly what `cyrs_hir::lower::lower_statement("MATCH")`
...
fn lower_statement_rejects_rel_first_pattern_part() {
    use cyrs_hir::{Direction, RelLength};
    let span = HirSpan::default();
    let mut stmt = Statement::new(span);
    stmt.clauses.push(Clause::Match {
        id: HirId::DUMMY,
