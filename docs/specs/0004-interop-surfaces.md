...
runtimes — WebAssembly (`cypher-wasm`), a stable C ABI (`cypher-ffi`), and
Python (`cypher-py`) — and pins stability commitments for each so downstream
...
`web-lsp` feature on `cypher-lsp`. Every new surface is a **thin adapter**:
...
thin adapters that expose the existing `cypher-lang-services` engine — no
...
harness assert tree-sitter and `cypher-syntax` agree on well-formedness.

LSP-in-browser (`cypher-lsp` with `web-lsp`) is the smallest addition: it
reuses the WASM toolchain set up for `cypher-wasm` and lets the demo page
...
**2.1. §2.C3 ("no overlay crates") still holds.** `cypher-wasm`, `cypher-ffi`,
...
| -------------- | ------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `cypher-wasm`  | `wasm32-unknown-unknown` binding: agent wire protocol as JS callables     | `cypher-lang-services`, `cypher-db`, `cypher-diag`, `cypher-fmt`, `wasm-bindgen`, `serde-wasm-bindgen` |
| `cypher-ffi`   | `cdylib` + `staticlib` exporting a stable C ABI + cbindgen-generated header | `cypher-lang-services`, `cypher-db`, `cypher-diag`, `cypher-fmt`                |
| `cypher-py`    | PyO3 extension module shipped as a `maturin`-built wheel                  | `cypher-lang-services`, `cypher-db`, `cypher-diag`, `cypher-fmt`, `pyo3`        |
...
`cypher-lang-services` and translate results back. Spec 0001 §3.1's
thin-shell clause on binary crates applies to these three in full. A clippy
lint on `unused_crate_dependencies` plus a CI grep for `cypher_ast::` and
`cypher_syntax::` inside these three crates keeps the rule honest.
...
§3.1's allowed edges (`cypher-lang-services` depends on the library trunk),
...
field (sealed by `cy-2i9`) is the sole versioning axis. `cypher-wasm`
...
`#![forbid(unsafe_code)]` workspace-wide. `cypher-ffi` is granted a
crate-local exception — `unsafe` is required for raw-pointer handling at
the ABI. The waiver is justified in `cypher-ffi/README.md`, enforced with
...
`cypher-ffi/include/cypher.h`; `--check` mode asserts the committed header
...
**6.1. Binding choice.** `cypher-py` binds `cypher-lang-services` directly
via PyO3; it does **not** go through `cypher-ffi`. Rationale: PyO3 handles
...
**6.3. Type stubs.** `cypher-py` ships a hand-written `cypher.pyi` alongside
...
**7.1. Feature gate.** `cypher-lsp` gains a `web-lsp` Cargo feature. When
...
contract is untouched. Clients built for stdio `cypher-lsp` can reuse their
...
`cypher-syntax` use different grammar models, and locking node shapes
...
runs this on every PR that touches either `cypher-syntax/` or
`tree-sitter-cypher/`.
...
promoted. `cypher-wasm` and LSP-Web (§7) reuse these rails verbatim.
...
`CHANGELOG.md` entry in `cypher-ffi/`.

**9.3. Python API.** `cypher-py` follows PEP 440 SemVer. Public classes
...
`cypher-hir::Expr`, etc.) may grow variants inside a minor; the `cypher.pyi`
...
that touches `cypher-wasm/` or its upstream deps.
...
**10.3. Python.** `pytest` under `crates/cypher-py/tests/` exercises
...
- **crates.io publishing** of `cypher-wasm`, `cypher-ffi`, `cypher-py` is
...
- **PyPI publishing** of `cypher-py` is deferred to `cy-zgz`. Package
  name reservation is covered by §13 below.
- **npm publishing** of the JS wrapper around `cypher-wasm` is deferred —
...
- **LSP-Web as its own crate.** If the `web-lsp` feature's transport
  shim grows past ~200 LOC, promote it to a sibling `cypher-lsp-transport`
  crate (see spec 0001 §3.1 thin-shell precedent). Revisit after
  `cy-od5.5` has landed an initial implementation.
- **Grammar generation from ungrammar.** Can `cypher-ast/cypher.ungrammar`
