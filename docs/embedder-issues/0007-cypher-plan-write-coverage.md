# 0007 — `cyrs-plan` write-side coverage parity with the embedder
...
cyrs's `cyrs-plan` exposes `WriteOp` (spec §12.1) but the public docs
...
If `cyrs-plan` covers, say, 60% of the write surface the embedder
